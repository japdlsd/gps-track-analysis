...
# 5 states: car, car_stop, walk_stop, walk + walk_elevation; variables: speed, bearing + elevation
...
for (i in 1:length(datasets)) {
  ...
  scatterplot3d(datasets[[i]]$lon, datasets[[i]]$lat, (datasets[[i]]$t - datasets[[i]]$t[1])/60, color=cols, cex.symbols = 0.1, xlab = NA, ylab=NA, zlab=NA, angle=60)
  #plot(xs, ys, col=cols, cex=0.1, main=paste("D", i, "#p:", pointnum, "h:", total_time), xlab=NA, ylab=NA)
}
```
