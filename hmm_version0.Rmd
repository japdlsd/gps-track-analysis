...
```{r def_hmm0, cache=TRUE}

init_prob <- list(walk=0.5, car=0.5)
...
emission_pp_f <- list(walk = walk_em, car = car_em)
...
```{r eval_viterbi, cache=TRUE, dependson=c("def_viterbi", "preprocess_data_geodiff", "def_hmm0")}
vres <- list()
for (i in 1:length(ddata)) {
